...
function algoBFS(fname, vD, vA)
    ...
    queue = Queue{Tuple{Int64, Int64}}() #simple queue, first seen first.
    parent = Dict{Tuple{Int64, Int64}, Tuple{Int64, Int64}}() #tracking
    enqueue!(queue, vD)
    parent[vD] = (-1, -1)
...
function algoDijkstra(fname, vD, vA)
    states = 0
    map, h, w = readmap(fname)
    L = PriorityQueue{Tuple{Int64, Int64}, Int64}()#priority to account for heuristic
    cost2 = fill(MAX, (h, w))#additive cost
    parent = Dict{Tuple{Int64, Int64}, Tuple{Int64, Int64}}()#tracking
...
function algoGlouton(fname, vD, vA)
    states = 0
    map, h, w = readmap(fname)
    #like Dijkstra but with distance
    ...
    while !(isempty(L))
        ...
        for (hr, vr) in direct
            ...
            if inbound
                ...
                priotot = dist + cost2 #heuristic g(x) + h(x)
